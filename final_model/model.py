...
#tokenizer = AlbertTokenizer.from_pretrained('albert-large-v2', use_fast=True)
...
#model = AlbertForSequenceClassification.from_pretrained('albert-large-v2', num_labels=len(class_names))
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model.to(device)

# Training and evaluation loop
optimizer = AdamW(model.parameters(), lr=0.0000002)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        if best_acc < accuracy and accuracy>0.8:
            best_acc = accuracy
            torch.save(model.state_dict(), f'D:\\model\\robert_model_L_{optim}_{epoch}_{best_acc}.pt')
        elif accuracy > 0.845:
            torch.save(model.state_dict(), f'D:\\model\\good model\\robert_model_L_{optim}_{epoch}_{best_acc}.pt')
